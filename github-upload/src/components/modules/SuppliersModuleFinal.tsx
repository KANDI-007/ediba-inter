...
const SuppliersModuleFinal: React.FC = () => {
  console.log('SuppliersModuleFinal - Component rendering');
  const { addSupplierInvoice, suppliersList: contextSuppliers, addSupplier, updateSupplier, deleteSupplier } = (typeof useData === 'function' ? useData() : ({} as any));
  // Garde-fous pour éviter écran blanc si le contexte n'est pas prêt
  const safeAddSupplierInvoice = (typeof addSupplierInvoice === 'function'
    ? addSupplierInvoice
    : (inv: any) => {
        console.warn('addSupplierInvoice indisponible (fallback no-op)');
        return { id: `SINV-${Date.now()}`, ...inv };
      }
  );
  ...
  // Tous les 10 fournisseurs fréquents par défaut
  ...
  // Utiliser les données du contexte si dispo, sinon les données par défaut
  const suppliersList = (Array.isArray(contextSuppliers) && contextSuppliers.length > 0) ? contextSuppliers : defaultSuppliers;
  ...
  // Normaliser produits: toujours un tableau de chaînes
  ...
  const handleUpdateSupplier = (id: string, updates: any) => {
    ...
    // Ici on pourrait ajouter la logique pour mettre à jour
  };

  const handleDeleteSupplier = (id: string) => {
    console.log('Deleted supplier:', id);
    // Ici on pourrait ajouter la logique pour supprimer
  };
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button 
              onClick={() => { 
                setEditingId(null); 
                setForm({ raisonSociale: '', nif: '', rccm: '', adresse: '', telephone: '', email: '', regimeFiscal: 'Réel Normal', produits: '' }); 
      ...
      <SupplierInvoiceForm suppliersList={suppliersList} addSupplierInvoice={safeAddSupplierInvoice} />
            ...
            <div className="p-6">
              <div className="flex items-start justify-between mb-4">
                ...
                <div className="flex space-x-1">
                  <button className="p-2 text-gray-400 hover:text-sky-600 hover:bg-sky-50 rounded-lg transition-colors duration-200" onClick={() => {
                    setEditingId(supplier.id as unknown as string);
                  ...
                  <button className="p-2 text-gray-400 hover:text-green-600 hover:bg-green-50 rounded-lg transition-colors duration-200" onClick={() => {
                    setEditingId(supplier.id as unknown as string);
                    setForm({ raisonSociale: supplier.raisonSociale, nif: supplier.nif, rccm: supplier.rccm || '', adresse: supplier.adresse || '', telephone: supplier.telephone || '', email: supplier.email || '', regimeFiscal: supplier.regimeFiscal || 'Réel Normal', produits: normalizeProduits(supplier.produits).join(', ') });
                    setShowAddModal(true);
                  }}>
...
export default SuppliersModuleFinal;

// Composant pour créer une nouvelle facture fournisseur
const SupplierInvoiceForm: React.FC<{ suppliersList: any[], addSupplierInvoice: (invoice: any) => any }> = ({ suppliersList, addSupplierInvoice }) => {
  ...
  const saveInvoice = () => {
    ...
    try {
      const saved = addSupplierInvoice(invoice);
      console.log('Facture fournisseur sauvegardée:', saved);
      ...
      console.error('Erreur lors de la sauvegarde de la facture:', e);
      alert(`Erreur lors de la sauvegarde de la facture: ${e?.message || e}`);
    }
  };
