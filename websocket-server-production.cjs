// Serveur WebSocket optimisé pour production EDIBA-INTER
...
io.on('connection', (socket) => {
  ...
  socket.on('user_connected', (userData) => {
    ...
    console.log(`✅ ${userData.username} ajouté à la liste (${connectedUsers.size} utilisateurs)`);
  });
  ...
  socket.on('send_message', (messageData) => {
    console.log(`💬 Message reçu de ${socket.id}:`, messageData.content);
  ...
  socket.on('disconnect', (reason) => {
    ...
    if (user) {
      ...
      console.log(`✅ ${user.username} retiré de la liste (${connectedUsers.size} utilisateurs restants)`);
    }
  });
...
server.listen(PORT, () => {
  ...
  console.log(`🌐 CORS configuré pour Netlify et localhost`);
});
